...
| `remember` | Persist or delete one project-scoped durable note |
| `recall` | Read one note or a sorted paginated page of durable notes; continuations are pinned by `snapshot_hash` so concurrent changes fail explicitly instead of shifting pages; optionally include the persisted plan |
...
For project work, the agent calls `chatgpt_turn_init` exactly once at the beginning of each new user turn, before other project tools or a project-state-dependent answer. On later turns it passes the `[ref:...]` token from the nearest preceding CodexBridge assistant final response as `previous_turn_ref`. After a successful call it must not call `chatgpt_turn_init` again until the user sends another message.

Each successful new turn issues a compact server-generated UUIDv7 `turn_ref`. A full returned brief contains the exact final-answer marker for that turn, for example `[ref:r_...]`; compact receipts carry the same marker, and project-related final responses must end with it. `previous_turn_ref` links the new turn to its parent. For one native conversation, `(native_key, previous_turn_ref)` is unique: retrying the same parent is idempotent and returns the already-created child `turn_ref` instead of extending the chain twice.

A new ChatGPT conversation under the same `openai/subject` may branch from a prior `turn_ref` and inherit that reference's effective project without repeating `project_key`. Turn references are subject-scoped in SQLite, so a reference from another OpenAI subject cannot be used as a project-join capability.

The structured result also reports `workspace_state`:

- `new` — this ChatGPT conversation had no persisted project binding, so a new isolated effective project was bound (unless a new alias was being created for it);
- `existing` — the conversation already had the same persisted effective project, which is rehydrated with its current saved state, skills, and instructions;
- `joined` — a fresh/differently-bound conversation explicitly joined an already existing shared `project_key` alias.

Every call rebuilds two independently hashed layers. The **instruction context** contains the runtime environment, skill/gateway catalogues, and project instructions and is SHA-256 hashed as `instruction_hash`. The **project state snapshot** contains a bounded presentation of durable memory plus the persisted plan and is hashed separately as `state_hash`; the semantic memory hash covers all stored notes, including notes beyond the first recall page. The turn-specific protocol and `turn_ref` are excluded from both hashes. SQLite stores both hashes for each turn and compares them with the hashes stored on `previous_turn_ref`.

On the first project turn, on a branched conversation, or when the instruction layer changed, `instructions_changed=true` and `brief` contains the refreshed instruction context, current bounded state snapshot, and turn protocol. If only memory/plan changed, `instructions_changed=false`, `state_changed=true`, `brief=null`, and `state_update` carries only the current bounded state snapshot. If neither hash changed, both `brief` and `state_update` are null. Routine plan/memory updates therefore do not re-inject the full coding brief.

The transport still does not provide a trustworthy user-message/turn identifier. Therefore CodexBridge can hard-enforce idempotency for duplicate calls that carry the same `previous_turn_ref`, and it rejects a later call on an already-bound conversation when the required parent ref is omitted, but it cannot detect a later user message for which the model never calls `chatgpt_turn_init` at all. The “call once at the start of every project-bearing user turn” requirement remains an agent protocol contract; no timing heuristic is used.
...
Conversation bindings, aliases, durable notes, plans, and turn references remain in one SQLite database under the metadata area. SQLite runs in WAL mode. CodexBridge currently starts from a fresh schema-v4 database and does not migrate older development schemas. CodexBridge uses one dedicated serialized writer connection plus a small pool of four query-only reader connections, so independent reads can progress concurrently and WAL readers can continue while the single writer commits. Writes remain intentionally ordered through one writer because SQLite itself permits only one writer at a time and turn-initialization transactions depend on deterministic compare-and-commit semantics.

Working memory is bounded per effective project: at most 512 notes, 256 bytes per key, 256 KiB per value, and 16 MiB total. A normal recall page is capped at 128 entries / 1 MiB and is sorted lexicographically by key. The first paginated `recall` response includes a semantic `snapshot_hash`; every continuation must send that same hash with `next_offset`. If notes were inserted, updated, or deleted between pages, the continuation returns `PAGINATION_STALE` instead of silently repeating or skipping rows from a shifted `OFFSET` window. Plans are capped at 100 items and 256 KiB total, and `update_plan` with an empty `plan` removes the persisted plan entirely. These limits reject oversized state rather than silently evicting earlier notes or plan content.

Turn-reference creation is part of the same transaction as the project-binding commit, together with subject scope, parent reference, `instruction_hash`, and `state_hash`, so a failed turn init cannot persist only one half of that boundary. The memory presentation page, complete semantic memory hash, and current plan used by `chatgpt_turn_init` are read from one SQLite read transaction, so the committed `state_hash` cannot describe a different database revision than the visible state snapshot. The instruction hash and returned brief are likewise derived from one loaded instruction context rather than independently re-reading project instructions. Deterministic duplicate continuation retries replay the brief/state snapshot committed for that turn even if durable state changes afterward. Rewind/checkpoint restoration is intentionally not implemented yet; `turn_ref` is already persisted as the stable anchor that a later snapshot layer can attach to.
