# CodexBridge Documentation

Detailed reference for CodexBridge. For the minimal ChatGPT setup flow, see [README.md](README.md).
...
- terminal resize and interrupt/terminate/kill signals; PTY interrupt writes Ctrl-C, Unix non-TTY interrupt/terminate map to SIGINT/SIGTERM, hidden Windows non-TTY processes reject `interrupt` because they have no usable console, Windows `terminate` uses non-forced `taskkill.exe /T` with a forced fallback when graceful tree termination fails, and `kill` forcefully terminates the tree;
...
Process results expose `completion_reason` (`running`, `exited`, `signaled`, `timed_out`, `cancelled`, or `failed`), nullable `exit_code`, `process_deadline_exceeded`, and an optional wait `error`. `completion_reason` is the authoritative process lifecycle state: `completion_reason=timed_out` means the subprocess timed out, while `process_deadline_exceeded=true` only records that its configured wall-clock deadline was crossed. Neither field describes the MCP/HTTP request itself. A requested signal does not by itself make a successful graceful exit `signaled`: if the process handles SIGTERM/SIGINT and exits with code 0, the authoritative result is `completion_reason=exited` plus `requested_signal=...`. On Unix, both pipe-backed commands and PTY sessions preserve the native wait status, so actual signal termination is reported as `completion_reason=signaled` with the signal number instead of being inferred from a nonzero exit code. CodexBridge does not use a synthetic `exit_code=-1` for signal/timeout cases. When the requested wall-clock deadline is reached, the waiter allows a bounded one-second completion grace: if the process is already finishing, its real exit status is preserved with `process_deadline_exceeded=true`; if it remains alive, CodexBridge terminates the process tree and returns `completion_reason=timed_out` with `process_deadline_exceeded=true`.
...
- Per-project concurrency permit entries and audit project-activity entries are both memory-bounded to 4096 entries. The permit registry evicts only fully idle project entries when it reaches capacity and fails closed with `SERVER_BUSY` if every entry is active; the audit activity cache evicts an existing entry when necessary to remain within its cap.
